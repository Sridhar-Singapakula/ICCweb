...
router.get('/:id/amountDeposited',[validObjectId,auth], async (req, res) => {
  try {
    ...
    // Get the amountDeposited IDs from the client document
    const amountDepositedIds = client.amountDeposited;
    
    // Fetch the amountDeposited details using the IDs
    const amountDepositedDetails = await AmountDeposited.find({
      _id: { $in: amountDepositedIds }
    });
    
    res.json(amountDepositedDetails);
  } catch (error) {
    console.error(error);
