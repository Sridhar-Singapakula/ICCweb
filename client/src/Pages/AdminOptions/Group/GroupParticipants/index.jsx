...
const GCGroupResult = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsFetching(true);
      const url = process.env.REACT_APP_API_URL + '/GCgroupparticipants'; // Adjust the API URL accordingly
      await axiosInstance.post(url, data);

      setIsFetching(false);
      toast.success('GC Group participants added successfully');
    } catch (error) {
      setIsFetching(false);
      if (
        error.response &&
