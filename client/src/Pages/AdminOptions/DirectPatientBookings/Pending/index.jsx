...
const App = () => {
  ...
  const transformData = () => {
    return patients
    .filter((patient) => {
      const anyTestPending = patient.tests.some((test) => test.status.status === 'pending');
      const anyPackagePending = patient.packages.some((p) => p.status.status === 'pending');
      
      return anyTestPending || anyPackagePending;
    })
